...
$url = 'https://dotnetcli.blob.core.windows.net/dotnet/beta/Installers/1.0.0.001598/dotnet-win-x8.1.0.0.001598.exe' # download url
$url64 = 'https://dotnetcli.blob.core.windows.net/dotnet/beta/Installers/1.0.0.001598/dotnet-win-x64.1.0.0.001598.exe' # 64bit URL here or remove - if installer is both, use $url
...
$packageArgs = @{
  ...
  #MSI
  silentArgs    = "/quiet /install /norestart /log ""$env:TEMP\chocolatey\$packageName\install.log""" # ALLUSERS=1 DISABLEDESKTOPSHORTCUT=1 ADDDESKTOPICON=0 ADDSTARTMENU=0
  validExitCodes= @(0, 3010, 1641)
  #OTHERS
  #silentArgs    ='/quiet' # "/s /S /q /Q /quiet /silent /SILENT /VERYSILENT -s" # try any of these to get the silent installer
  #validExitCodes= @(0) #please insert other valid exit codes here

  # optional
  registryUninstallerKey = 'dotnet' #ensure this is the value in the registry
  checksum      = ''
